...
def qLearning(env, num_episodes: int, discount: float, alpha: float, epsilon: float, epsilon_decay: float, Qvalues):
    """
        Returns:
...
# epsilon = 0.01 # could be lower as the agent learns
alpha = 0.2  # learning rate
discount = 0.8  # discount factor for future rewards (gamma)
Qvalues_dictionary = ''
# comment after you remove file and it is your first try
with open(r'Qvalues_dictionary.txt', 'r') as f:
    Qvalues_dictionary = ujson.load(f)

# Qvalues[(state, action)] = Qvalue
Qvalues = utils.Counter(Qvalues_dictionary)
observation, info = env.reset(seed=42)
learned_Qvalues = qLearning(env, 5005, discount, alpha, epsilon, 0.99, Qvalues)
# learned_Qvalues = qLearning(env, 10, discount, alpha, epsilon, 0.99, Qvalues)
# save Qvalues in a file
Qvalues_dictionary = dict(learned_Qvalues)
# with open('Qvalues_dictionary.txt', 'w') as file:
